...
import { fetchAiDemoTradePrefill, type AiDemoTradePrefill } from "../lib/aiFuturesApi";
import {
  applyMarketPrice,
...
export function DemoTrade() {
  ...
  const [aiPrefill, setAiPrefill] = useState<AiDemoTradePrefill | null>(null);
  const [aiPrefillMessage, setAiPrefillMessage] = useState<string | null>(null);
  const [isAiPrefillLoading, setIsAiPrefillLoading] = useState(false);
  const currentPriceRef = useRef<number | null>(null);
  const pendingMarketStateToPersistRef = useRef<DemoTradeState | null>(null);
  const consumedAiPlanRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    const planId = searchParams.get("aiPlan");
    if (!planId || !isHydrated || isAuthLoading || consumedAiPlanRef.current === planId) return;
    consumedAiPlanRef.current = planId;
    const clearPlanParameter = () => {
      const next = new URLSearchParams(searchParams);
      next.delete("aiPlan");
      setSearchParams(next, { replace: true });
    };

    if (demoState.openPosition || demoState.pendingLimitOrder) {
      setAiPrefillMessage("The AI practice plan was not imported because this demo account already has an open position or pending order.");
      clearPlanParameter();
      return;
    }

    let active = true;
    setIsAiPrefillLoading(true);
    setAiPrefillMessage(null);
    void fetchAiDemoTradePrefill(planId)
      .then((plan) => {
        if (!active) return;
        setAiPrefill(plan);
        setOrderType("limit");
        setLimitPrice(plan.entryPrice);
        setMarginMode("isolated");
        setQuantityUnit("usdt");
        setAmount(plan.notional);
        setLeverage(String(plan.leverage));
        setIsBracketEnabled(true);
        setStopLossInputMode("price");
        setStopLoss(plan.stopLoss);
        setTakeProfitInputMode("price");
        setTakeProfits(plan.takeProfits.map((target, index) => ({
          id: `ai-tp-${index + 1}`,
          price: target.price,
        ...
        setFormErrors([]);
        setAiPrefillMessage(
          `Imported a ${plan.direction.toUpperCase()} isolated-margin practice plan. Review every field, then explicitly choose Open ${plan.direction === "long" ? "Long" : "Short"}.`
        );
      })
      .catch((error) => {
        if (active) setAiPrefillMessage(error instanceof Error ? error.message : "AI practice plan could not be imported.");
      })
      .finally(() => {
        if (active) {
          setIsAiPrefillLoading(false);
          clearPlanParameter();
        }
      });
    return () => { active = false; };
  }, [demoState.openPosition, demoState.pendingLimitOrder, isAuthLoading, isHydrated, searchParams, setSearchParams]);
  ...
  return (
    <main className="page page-stack demo-trade-page">
      ...
      {isAiPrefillLoading && <p className="notice-box">Loading protected AI practice plan…</p>}
      {aiPrefillMessage && (
        <section className={aiPrefill ? "notice-box ai-demo-prefill-banner" : "warning-box ai-demo-prefill-banner"}>
          <div>
            <strong>{aiPrefill ? "AI practice plan imported for review" : "AI practice plan not imported"}</strong>
            <p>{aiPrefillMessage}</p>
            {aiPrefill && <small>Source setup expires {new Date(aiPrefill.expiresAt).toLocaleString()}. Importing this plan did not place or persist an order.</small>}
          </div>
        </section>
      )}
...
function DemoTradeChart({
  ...
  const overlayLines = useMemo(
    () => buildOverlayLines(position, pendingLimitOrder),
    [pendingLimitOrder, position]
  );
  ...
  return (
    <FuturesCandlestickChart
      ...
      timeframe={timeframe}
      timeframeOptions={demoTradeTimeframes}
      onTimeframeChange={onTimeframeChange}
      ariaLabel="Custom BTC USDT demo trading chart"
      isLoading={!candles.length}
      loadingLabel="Loading custom BTC chart"
      loadingDescription="Live candles will appear here when market data is available."
      readOnly
    />
  );
}
...
function buildOverlayLines(
  position: DemoOpenPosition | null,
  pendingLimitOrder: DemoPendingLimitOrder | null
): FuturesChartOverlayLine[] {
  const lines: FuturesChartOverlayLine[] = [];
  if (!position && pendingLimitOrder?.limitPrice) {
    lines.push({
      id: "pending-limit",
      label: "Limit",
      price: pendingLimitOrder.limitPrice,
      tone: "pending"
    });
  }
  ...
  if (position.stopLoss > 0) {
    lines.push({ id: "position-stop-loss", label: "SL", price: position.stopLoss, tone: "danger" });
  }
  ...
  position.takeProfits.forEach((takeProfit, index) => {
    lines.push({
      ...
      label: `TP${index + 1}`,
      price: takeProfit.price,
      tone: takeProfit.isHit ? "hit" : "target"
    });
  });
  return lines;
}
